...
- **Import historical usage into the Energy Dashboard.** Each business agreement now exposes two buttons: **Import historical electricity data** (consumption + injection) and **Import historical gas data** (Settings > Devices & Services > ENGIE Belgium > *the account*). First press pulls hourly usage from ENGIE back to the business agreement's start date and writes it into Home Assistant's long-term statistics under per-BAN external statistic IDs (`engie_be:{BAN}_consumption`, `_injection`, `_gas`). Subsequent presses only fetch new hours since the last press. The values appear in the Energy Dashboard's electricity and gas source pickers. Hide the button you don't need if you only have one energy type.
