...
- **`engie_be.import_history` service** exposes the same import with optional `fuel` (consumption / injection / gas / any combination), `start_date`, and `end_date` fields (Developer Tools > Actions > *ENGIE Belgium: Import historical usage*, or from an automation). Target one or more business-agreement devices; omit all fields for auto mode (all streams, incremental delta); pick one or more fuels to limit the import; provide dates to re-import a specific window, overwriting existing hourly rows in place.
- **`engie_be.clear_import_history` service** deletes imported statistic streams for the targeted business-agreement device. Optional energy-type field to clear only selected stream(s). The next `Import historical usage` call for the same device and energy type backfills everything again from the business agreement's start date. Useful when ENGIE republishes historical data after the fact.
