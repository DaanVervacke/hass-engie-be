...
as sensors, binary sensors, events, and calendar events.
...
- Event entities that record state transitions (EPEX price sign flips, Happy Hours activation, TOU slot changes, authentication loss) in the HA logbook
...
> visible in **Tools** > **States** and in the device name
...
**Tools** > **States** or on the account device page.

### Events

Six event entities record state transitions in the Home Assistant
logbook. Each entity watches one or more sibling sensors and re-fires
their transitions as HA events with timestamps.

| Entity | Entity ID | Records |
|---|---|---|
| EPEX hourly price events | `event.engie_belgium_{BAN}_epex_events` | Hourly EPEX price going negative or returning positive |
| EPEX quarter-hourly price events | `event.engie_belgium_{BAN}_epex_events_quarter_hourly` | Quarter-hourly EPEX price going negative or returning positive |
| Happy Hours events | `event.engie_belgium_{BAN}_happy_hours_events` | Happy Hours window activating or deactivating |
| TOU events | `event.engie_belgium_{BAN}_tou_events` | TOU offtake/injection slot changes and optimal-slot transitions |
...
**Import historical usage** action from **Settings** > **Tools** > **Actions**.
...
business-agreement devices and can be called from **Tools** >
...
- ENGIE does not expose historical energy prices through the API. The integration can only report the currently active price period. Historical sensor data is what Home Assistant's own recorder stores.
- Happy Hours history starts when the integration is installed. ENGIE's Smart App and API surface only the next upcoming Happy Hours window. The integration records each window it observes locally, so past windows are recoverable inside HA only from the point of install onwards. ENGIE also publishes an archive of past Happy Hours at [engie.be/nl/happyhours/overzicht](https://www.engie.be/nl/happyhours/overzicht) if you need older data.
...
- Solar Surplus has eligibility criteria and is still being rolled out. ENGIE lists four requirements: solar panels that inject into the grid, a digital meter, an Easy, Flow, Empower or Direct electricity contract, and use of the ENGIE Smart App. ENGIE also states it is testing the feature with a small group before releasing it to everyone who meets those criteria. If you have solar panels but no Solar Surplus entities, check the four requirements first. If you meet all of them, your account is most likely not in the rollout yet. See [engie.be/nl/solar-surplus](https://www.engie.be/nl/solar-surplus).
- The Energy dashboard's solar forecast shows expected injection, not production. Home Assistant calls that card "Solar production forecast", but injection is the only solar figure ENGIE publishes: what your panels generate minus what your household is using at that moment. The card therefore reads lower than your actual production by however much you consume on site, and drops to zero for hours where you expect to consume everything you generate.
- Both hourly and quarter-hourly EPEX sensors are created for all dynamic contracts. The actual billing granularity of your contract may differ from what these sensors report.
- The same ENGIE credentials cannot be shared with engie.be or the ENGIE Smart App without triggering frequent re-authentication prompts. This is an ENGIE platform constraint, not a Home Assistant limitation. See [Prerequisites](#prerequisites).
- The integration requires MFA to be enabled on the ENGIE account. Accounts without MFA (e.g. older sub-accounts) are not supported.
- The integration only reads data from the ENGIE API and never modifies your account, contracts, or settings.
